...
if __name__ == "__main__":
    ...
    #     columns=['Embryo Name', 'Time Point', 'Raw Signal Power', 'Raw Noise Power', 'Pseudo Signal Power',
    #              'Pseudo Noise Power', 'Raw SNR', 'Pseudo SNR'])
    # slice_num = 68
    # size_3d = [256, 356, 160]
    #
    # embryo_names = ['181210plc1p2']
    ...
    # pseudo_nuclei_folder = r'F:\TifsForLineage'
    ...
    #
    #             img = np.asanyarray(Image.open(os.path.join(pseudo_nuclei_folder,embryo_name,'tif', raw_file_name)))
    #             out_stack.insert(0, img)
    #         img_stack = np.transpose(np.stack(out_stack), axes=(1, 2, 0))
    #         pseudo_nuc_volume = resize(image=img_stack, output_shape=size_3d, preserve_range=True, order=1).astype(
    #             np.int16)
    #
    ...
    #         # raw_nuc_volume=resize(image=raw_nuc_volume,output_shape=size_3d,preserve_range=True,order=1)
    ...
    #         # gt_nuc_volume=resize(image=gt_nuc_volume,output_shape=size_3d,preserve_range=True,order=0,anti_aliasing=False)
    #
    #
    #         pseudo_signal_power = pseudo_nuc_volume[gt_nuc_volume > 0].sum()
    #         pseudo_total_power = pseudo_nuc_volume.sum()
    #         pseudo_noise_power = pseudo_total_power - pseudo_signal_power
    #
    ...
    #                                                                    raw_noise_power, pseudo_signal_power,
    #                                                                    pseudo_noise_power,
    #                                                                    raw_signal_power / raw_noise_power,
    #                                                                    pseudo_signal_power / pseudo_noise_power]
    # df_SNR_nuc_enhancement.to_csv('SNR_enhanced_nuc181210plc1p2.csv')
    # ==============================================================================

    # =========================plot SNR =========================================
    # embryo_naming={'181210plc1p2':'WT_C_Sample6', '200326plc1p3':'WT_Sample4','200326plc1p4':'WT_Sample5'}
    # ONLYONE='200326plc1p4'
    # df_SNR_nuc_enhancement=pd.read_csv(r'./SNR_enhanced_nuc{}.csv'.format(ONLYONE))
    ...
    #     df_SNR_nuc_enhancement_plotting.loc[len(df_SNR_nuc_enhancement_plotting)]=[embryo_naming[embryo_name],tp,SNR_enhanced,'Pseudo']
    #     df_SNR_nuc_enhancement_plotting.loc[len(df_SNR_nuc_enhancement_plotting)]=[embryo_naming[embryo_name],tp,SNR_raw,'Raw']
    # df_SNR_nuc_enhancement_plotting=df_SNR_nuc_enhancement_plotting.loc[df_SNR_nuc_enhancement_plotting['Sample']==embryo_naming[ONLYONE]]
    #
    #
    # hue_order_list = [
    #         'Raw',
    ...
    #
    # plt.xlabel("Imaging Time Point", size=20)
    ...
    # max_times = [210,220, 195]
    # embryo_names = ['200326plc1p4']
    # max_times = [195]
    #
    # packed_and_gt_nuc_path = r'F:\packed membrane nucleus 3d niigz'
    # pseudo_folder = r'F:\TifsForLineage'
    # raw_folder = r'E:\ProjectData\MembraneProject\AllRawData'
    #
    # for idx_emb, embryo_name in enumerate(embryo_names):
    #     for tp in range(1,max_times[idx_emb]+1):
    ...
    #             raw_file_name = "{}_L1-t{}-p{}.tif".format(embryo_name, str(tp).zfill(3), str(i_slice).zfill(2))
    #
    #             img = np.asanyarray(Image.open(os.path.join(pseudo_folder, embryo_name, 'tif', raw_file_name)))
    #             out_stack.insert(0, img)
    #         img_stack = np.transpose(np.stack(out_stack), axes=(1, 2, 0))
    ...
    #
    #         df_SNR_nuc_enhancement_plotting.loc[len(df_SNR_nuc_enhancement_plotting)] = [embryo_naming[embryo_name], tp, enhanced_mse,'Pseudo']
    #         df_SNR_nuc_enhancement_plotting.loc[len(df_SNR_nuc_enhancement_plotting)] = [embryo_naming[embryo_name], tp, raw_mse,'Raw']
    #     df_SNR_nuc_enhancement_plotting.to_csv('MSE_enhanced_nuc{}.csv'.format(embryo_name))

    # =============================plotting MSE figures========================================

    embryo_naming={'181210plc1p2':'WT_C_Sample6', '200326plc1p3':'WT_Sample4','200326plc1p4':'WT_Sample5'}
    ONLYONE='200326plc1p4'
    df_SNR_nuc_enhancement_plotting=pd.read_csv('MSE_enhanced_nuc{}.csv'.format(ONLYONE),index_col=[0])
    df_SNR_nuc_enhancement_plotting['PSNR']=df_SNR_nuc_enhancement_plotting['MSE'].apply(lambda x:20*math.log10(255/math.sqrt(x)))

...
def seperate_3dniigz_to_2dtif(para):
    ...
    for page_num in range(1, raw_img_shape[-1] + 1):
        ...
        raw_tif_file_name = '{}_L1-t{}-p{}.tif'.format(
            embryo_name, str(tp).zfill(3), str(raw_index).zfill(2))
        output_paths.append(os.path.join(target_tif_root_path, raw_tif_file_name))

    missing_paths = [path for path in output_paths if not os.path.exists(path)]
    skipped_count = len(output_paths) - len(missing_paths)
    if not missing_paths:
        return {
            'source': source_niigz_file_path,
            'created': 0,
    ...
    for page_num in range(1, raw_img_shape[-1] + 1):
        ...
        saving_tif_path = output_paths[page_num - 1]

        if os.path.exists(saving_tif_path):
            continue
